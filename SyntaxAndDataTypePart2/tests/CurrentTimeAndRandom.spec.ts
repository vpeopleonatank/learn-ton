...
describe('CurrentTimeAndRandom', () => {
    ...
    it.skip('should wait 10 sec', async () => {
        ...
        await currentTimeAndRandom.send(deployer.getSender(), { value: toNano('0.2') }, 'wait 10s');

        await sleep(10500);

        await currentTimeAndRandom.send(deployer.getSender(), { value: toNano('1.2') }, 'wait 10s');
    }, 20000);
